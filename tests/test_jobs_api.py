...
def test_worker_processes_queued_job():
    ...
    job_id = response.json()["id"]
    # 2. Enqueue the job (Producer via Celery)
    run_response = client.post(f"/jobs/{job_id}/run")
    assert run_response.status_code == 202
    assert run_response.json()["status"] == "queued"
    # 3. Execute the Celery Task directly:
    result = execute_job_task(job_id)
...
def test_job_failure_moves_to_dlq():
    ...
    # 1. Create a failing job (Division by zero!)
    response = client.post("/jobs", json={
        "type": "division",
        "payload": {"a": 10, "b": 0}
    })
    ...
    # 3. Execute Celery Task on its final attempt (retries=5 >= max_retries):
    queue = JobQueue()
    result = execute_job_task.apply(args=[job_id], retries=5).get()
